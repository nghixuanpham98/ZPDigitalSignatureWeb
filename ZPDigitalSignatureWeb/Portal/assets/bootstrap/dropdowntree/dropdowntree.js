...
(function($) {
    ...
    function RenderData(data, element){
        for(var i = 0 ; i< data.length ; i++){
            ...
            if(!element.is("li")){
                element.append('<li id="TreeElement' + globalTreeIdCounter + '"' + dataAttrs + '>' + (options.multiSelect ? '<i class="far fa-square select-box" data-name="' + data[i].value + '" aria-hidden="true"></i>' : '') + '<a href="'+((typeof data[i].href != "undefined" && data[i].href!=null)?data[i].href:'#')+'">'+data[i].title+'</a></li>');
                ...
                element.find("ul").append('<li id="TreeElement' + globalTreeIdCounter + '"' + dataAttrs + '>' + (options.multiSelect ? '<i class="far fa-square select-box" data-name="' + data[i].value + '" aria-hidden="true"></i>' : '') + '<a href="'+((typeof data[i].href != "undefined" && data[i].href!=null)?data[i].href:'#')+'">'+data[i].title+'</a></li>');
                if(data[i].data != null && typeof data[i].data !="undefined"){
                    $("#TreeElement"+globalTreeIdCounter).append("<ul style='display:none'></ul>");
                    $("#TreeElement"+globalTreeIdCounter).find("a").first().prepend('<span class="arrow">'+options.closedArrow+'</span>');
                    RenderData(data[i].data, $("#TreeElement"+globalTreeIdCounter).find("ul").first());
    ...
    $(options.element).on("click",".select-box",function(e){
        ...
        if($(this).hasClass("fa-square")){
            ...
            if(options.selectChildren){
                $(this).parents("li").first().find(".select-box").removeClass("fa-square");
                $(this).parents("li").first().find(".select-box").addClass("fa-check-square");
            }
        }else{
            //will unselect
            checked=false;
            ...
            if(options.selectChildren){
                $(this).parents("li").first().find(".select-box").addClass("fa-square");
                $(this).parents("li").first().find(".select-box").removeClass("fa-check-square");
                $(this).parents("li").each(function(){
                    $(this).find(".select-box").first().removeClass("fa-check-square");
                    $(this).find(".select-box").first().addClass("fa-square");
                });
            }
        }
    ...
    $(options.element).append('<button class="btn btn-info dropdown-toggle" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true"><span class="dropdowntree-name">'+options.title+'</span><span class="caret"></span></button>');
